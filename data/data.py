...
def add_intraday(year):
    ...
    while start_date < end_date:
        SE3_intraday = get_intraday_price(start_date)
        ...
        for d in SE3_intraday:
            if d['Columns'][4]['Name'] == '':
                ...
                try:
                    ...
                    SE3_hourly_prices[i] = float(
                        d['Columns'][4]['Value'].replace(',', '.'))
                except:
                    print('Nicht')
                    print(start_date.strftime("%Y-%m-%d"))
        intraday_prices['Intraday SE3'] = intraday_prices['Intraday SE3'] + \
            SE3_hourly_prices
...
def get_cap_data():
    # Elspot flow data
    cap = load_market_data(
        'elspot_dayahead/elspot-capacities-se', 2015, 'cap ')

    for y in range(2016, 2021):
        cap2 = load_market_data(
            'elspot_dayahead/elspot-capacities-se', y, 'cap ')
...
def load_dayahead_prices(year):
    ...
    if year == 2015:
        df = pd.read_csv('elspot_dayahead/elspot-prices_{}_hourly_eur.csv'.format(year),
                         sep=';', decimal=",", header=2, encoding='iso 8859-1', usecols=[0, 1, 5, 6])
    else:
        df = pd.read_csv('elspot_dayahead/elspot-prices_{}_hourly_eur.csv'.format(
            year), sep=';', decimal=",", header=2, usecols=[0, 1, 5, 6])
...
def get_intraday_price(date):
    day, month, year = date.strftime(
        '%d'), date.strftime('%m'), date.strftime('%Y')
    URL = "https://www.nordpoolgroup.com/api/marketdata/page/194?currency=,EUR,EUR,EUR&endDate={}-{}-{}&entityName=SE3".format(
        day, month, year)
    # A delay between each request was incorporated as not to affect the performance of their server.
    time.sleep(1)
    try:
        SE3_data = requests.get(url=URL).json()['data']['Rows']
    except:
        SE3_data = []
...
def combine_yearly_price_data():
    df = pd.read_csv('price_yearly/price_data_2015.csv', sep=',',
                     decimal=".", index_col='Delivery', parse_dates=['Delivery'])
    for year in range(2016, 2021):
        df2 = pd.read_csv('price_yearly/price_data_{}.csv'.format(year), sep=',',
                          decimal=".", index_col='Delivery', parse_dates=['Delivery'])
...
def load_market_data(path, year, prefix):
    df = pd.read_csv('{}_{}_hourly.csv'.format(path, year),
                     sep=';', decimal=",", header=2, encoding='iso 8859-1')
    df.rename(columns={"Unnamed: 0": "Date"}, inplace=True)
    df.fillna(0, inplace=True)
...
def get_eq_data(region, variable, tag, path_name, shortname):
    if not path.exists(path_name):
        ...
        forecasted.rename(columns={'{} {} 15min Forecast'.format(
            region, variable): '{} {}'.format(shortname, region)}, inplace=True)
        actual.fillna(method='ffill', inplace=True)
        actual.rename(columns={'{} {} H Actual'.format(
            region, variable): '{} {}'.format(shortname, region)}, inplace=True)
        ...
        cons = pd.read_csv(path_name, encoding="ISO-8859-1",
...
def combine_production_data():
    wp = get_eq_data('SE', 'Wind Power Production MWh/h',
                     'ecsr', 'production_data/wp.csv', 'wp')
    solar = get_eq_data('SE', 'Solar Photovoltaic Production MWh/h',
                        'ecsr', 'production_data/solar.csv', 'solar')
    nuclear = get_eq_data('SE', 'Nuclear Production MWh/h',
                          '', 'production_data/nuclear.csv', 'nuclear')
...
def get_cons_data(region):
    if not path.exists('consumption_data/cons.csv'):
        ...
        forecasted_cons.rename(columns={'{} Consumption MWh/h 15min Forecast'.format(
            region): 'cons {}'.format(region)}, inplace=True)
        actual_cons.fillna(method='ffill', inplace=True)
        actual_cons.rename(columns={
                           '{} Consumption MWh/h H Actual'.format(region): 'cons {}'.format(region)}, inplace=True)

        cons = pd.concat([actual_cons, forecasted_cons],
        ...
        cons = pd.read_csv('consumption_data/cons.csv', encoding="ISO-8859-1",
                           sep=',', decimal='.', index_col=0, parse_dates=True)
...
def combine_weather_data():
    temp = get_eq_data('SE', 'Consumption Temperature °C',
                       'ecsr', 'weather_data/temp.csv', 'temp')
...
if __name__ == "__main__":
    if not path.exists('feature_data.csv'):
        ...
        feature_data = pd.concat([production_data, consumption_data, capacity_data,
                                 seasonal_data, flow_data], ignore_index=False, axis=1)
        ...
        feature_data.to_csv('feature_data2.csv', index=True)
    else:
        feature_data = pd.read_csv('feature_data.csv', encoding="ISO-8859-1",
                                   sep=',', decimal='.', index_col=0, parse_dates=True)
    ...
    if not path.exists('price_data.csv'):
        ...
        price_data = pd.read_csv('price_data.csv', encoding="ISO-8859-1",
