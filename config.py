...
class Config(object):
   def __init__(self):
      ...
      self.min_lookback       = 1
      self.max_lookback       = 30 if self.model in ['LSTM', 'GRU', 'THD'] else 3
      self.output_size        = 24
      ...
      self.flow_len           = 1 if self.model in ['LSTM', 'GRU', 'THD'] else self.max_lookback*24

      self.price_dayahead_len = 1 if self.model in ['LSTM', 'GRU', 'THD'] else self.max_lookback*24
      self.price_intraday_len = 1 if self.model in ['LSTM', 'GRU', 'THD'] else (self.max_lookback*24-11)
